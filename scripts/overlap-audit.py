...
def strip_chrome(html):
    ...
    patterns = [
        r"<(script|style)\b.*?</\1>",
        # Футерная перелинковка (19.09.2026): один и тот же блок на всех
        # 53 страницах. Без этой строки любая пара статей даёт 60-110
        # «дословных повторов» — это текст блока, а не содержание статей.
        r"<!-- sf-nav:start -->.*?<!-- sf-nav:end -->",
        r"<nav\b.*?</nav>",
        r"<footer\b.*?</footer>",
        r"<header\b.*?</header>",
