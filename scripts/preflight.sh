...
# Сертификат, которому не доверяют браузеры: сервер отвечает 200, а человек
# видит красный экран «Подключение не защищено». Так себя ведёт secrets.tbank.ru
# с сертификатом УЦ Минцифры. Прецедент 19.09.2026: ссылка в «Публикациях»
# не открывалась месяцами, а preflight про внешние ссылки вообще молчал.
# Пуш не блокируем: чинить надо на чужой стороне, наше дело — предупредить читателя.
if [ -n "$FASTFLAG" ]; then
  warn "Сертификаты внешних ссылок: пропущено (--fast)"
else
  CERTBAD=$(echo "$AUDIT" | grep -c 'сертификат не признан')
  if [ "${CERTBAD:-0}" = "0" ]; then
    ok "Сертификаты внешних ссылок: чисто"
  else
    warn "ссылок с сертификатом, которому не доверяют браузеры: $CERTBAD"
    echo "$AUDIT" | grep 'сертификат не признан' | sed 's/^ */    /'
  fi
fi
