...
def make_drafts(chat, date, n, redo=False):
    ...
    # Статическая часть задания — первой, динамика (номер, дата, файлы) — в хвосте:
    # так префикс промпта одинаков для всех новостей и берётся из кэша.
    ok, err = run_agent("smm-copywriter", (
        "Напиши пост для канала по правилам своей роли. "
        "Исходник — запись дайджеста (поля title, source, link, angle, why_now). "
        "Готовый пост запиши в указанный ниже файл: только текст поста, "
        "без заголовков и комментариев. В канал ничего не отправляй.\n\n"
        f"Новость №{n}. Дайджест: .news/digest-{date}.json. "
        f"Файл для поста: .news/post-{date}-{n}.txt.{extra}"
    ))
    if not ok:
    ...
    ok, err = run_agent("scriptwriter", (
        "Напиши сценарий вертикального видео на 20–45 секунд по правилам "
        "своей роли. Исходник — запись дайджеста (поля title, link, angle, "
        "why_now). Особое внимание хуку в первые 3 секунды — новость свежая, "
        "на ней можно поймать внимание. Сценарий запиши в указанный ниже файл "
        "и никуда не отправляй — доставку сделает скрипт.\n\n"
        f"Новость №{n}. Дайджест: .news/digest-{date}.json. "
        f"Файл для сценария: .news/reels-{date}-{n}.txt."
    ))
    if not ok:
