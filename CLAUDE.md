...
## Внешние публикации (ожидается)
- Кейс «Как архитектор набрал 7000 подписчиков за неделю без рекламы» (про клиента @eldarmurakov) ПРИНЯТ к публикации в Бизнес-секретах Т-Банка (secrets.tbank.ru), редактор Анна. Статус на 03.07.2026: принято к публикации, выйдет до 14 июля 2026 включительно, ссылку пришлют в Telegram-бот «Редакция Блогов компаний». Когда ссылка придёт, сделать: 1) бэклинк с сайта на статью Т-Банка через точечный str_replace в подходящей статье блога; 2) анонс в Telegram-канал @artsiombahram через scripts/post-to-telegram.sh; 3) обновить CLAUDE.md — перенести запись в раздел выполненных внешних публикаций.
- ✅ ВЫШЛА 14.07.2026 — «Геймификация в премиум-нише: кейс роста архитектора без рекламы», Т-Бизнес секреты (медиа Т-Банка): https://secrets.tbank.ru/blogi-kompanij/gejmifikaciya-v-premium-nishe/ — это тот самый кейс @eldarmurakov, редакция сменила заголовок. Бэклинк с сайта проставлен: about.html, блок «Где меня найти» (карточка .link-card рядом с vc.ru). Осталось: анонс в @artsiombahram через scripts/post-to-telegram.sh.
...
4. Следующие статьи (кандидаты, проверять на дубль перед написанием):
   - Полное руководство по блокировкам Instagram 2026 (hub-страница, линкует существующие статьи кластера)
...
## Опубликованные статьи (33 статьи)
...
4. Перед новой статьёй: сверка на дубль + фактчек по быстрым темам (Telegram/WhatsApp/Meta/регуляторка).
5. Дифф перед пушем — Артём одобряет. publish-article.sh Артём запускает вручную сам.
