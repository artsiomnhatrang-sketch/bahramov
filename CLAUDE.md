...
- 23.08.2026: Instagram @bahramovartem добавлен в `sameAs` (Person на главной и в about.html, Organization на главной);
  в Organization также добавлен LinkedIn — списки соцпрофилей выровнены во всех трёх блоках JSON-LD.
  Повод: Google Search Console с 07.07.2026 позволяет подключать соцаккаунты как отдельные свойства
  (Instagram/TikTok/X/YouTube) и показывать по ним клики и показы из Google Поиска и Discover.
  Это только отчётность, на ранжирование не влияет. `sameAs` — то, что реально связывает сайт и профили
  в одну сущность. lastmod в sitemap намеренно не трогали: менялся только <head>.
